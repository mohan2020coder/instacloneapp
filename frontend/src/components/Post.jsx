...
const Post = ({ post }) => {
    ...
    const likeOrDislikeHandler = async () => {
        try {
            ...
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/post/${post._id}/${action}`, { withCredentials: true });
            if (res.data.success) {
                setPostLike(prevLike => liked ? prevLike - 1 : prevLike + 1);
    ...
    const commentHandler = async () => {
        try {
            const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/post/${post._id}/comment`, { text }, {
                headers: {
    ...
    const deletePostHandler = async () => {
        try {
            const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/post/delete/${post._id}`, { withCredentials: true });
            if (res.data.success) {
    ...
    const bookmarkHandler = async () => {
        try {
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/post/${post._id}/bookmark`, { withCredentials: true });
            if (res.data.success) {
                toast.success(res.data.message);
            }
        } catch (error) {
